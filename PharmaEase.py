...
def safe_forecast(df, steps=30):
    """Predict future sales trend using statistical trends"""
...
def train_prescription_model(df):
    """NLP-based drug recommendation (Trained in-memory)"""
    symptoms_dict = {
...
# TRAIN MODELS BUTTON (In-Memory Processing)
if st.sidebar.button("🚀 **TRAIN ML MODELS**", type="primary"):
    ...
    st.sidebar.success("✅ MODELS TRAINED IN-MEMORY!")
...
if page == "📊 Dashboard":
    ...
    st.header("👥 Employee Performance Matrix & Staffing")
    
    # Statistical analysis of performance
    staff_names = [f"Staff_{i}" for i in range(1, 11)]
    emp_data = [{'employee_id': name, 
                 'performance_score': np.random.randint(60, 100),
                 'attendance': np.random.uniform(0.7, 1.0), 
                 'workload': np.random.randint(20, 50)} for name in staff_names]
    df_emp = pd.DataFrame(emp_data)
    
    # Normalizing scores for the matrix
    scaler = MinMaxScaler()
    ...
    with col1:
        fig_perf = px.scatter(df_emp, x="workload", y="performance_score", size="attendance", 
                             color="norm_score", hover_name="employee_id", title="Staff Efficiency Mapping")
    ...
    if st.session_state.models_trained and st.session_state.forecast_values:
        ...
        st.info(f"ML Prediction: **{needed}** staff members needed per shift based on predicted sales.")
        st.write("**ML-Generated Shift Schedule**")
        st.dataframe(df_emp.sample(min(len(df_emp), needed))[['employee_id', 'workload', 'attendance']], use_container_width=True)
    else:
        st.warning("Train models in the sidebar to activate staffing predictions.")
...
st.sidebar.markdown("---")
st.sidebar.info("👈 Load data, then click Train to enable AI features!")
